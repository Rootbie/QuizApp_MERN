...
class Edit extends React.Component {
    ...
    async componentDidMount() {
        ...
        const res = await fetch(`http://localhost:3001/questions/${queId}`)
        const specificQue = await res.json()

        this.setState({
            text: specificQue.text,
            answers: specificQue.answers
        })
    ...
    handleSubmit = async (event) => {
        ...
        if (bool === true && fooz === true) {
            ...
            await fetch(`http://localhost:3001/questions/${queId}`, {
                method: 'PUT',
                                ...
                                {this.state.answers.map((option, i) => {
                                    return (
                                        <div className="answer" key={i}>
                                            <input type="text" name="answers" placeholder={`Option ${i + 1}`} value={option} onChange={(event) => this.handleOption(event, i)} />
                                            <div>
